...
export function demoData(): StockData[] {
  return ROWS.map((r) => {
    ...
    return {
      ...
      priceToBook: 1 + (pe % 7) * 0.6,
      priceToSales: 0.8 + (analysts % 6) * 0.9,
      enterpriseToEbitda: 6 + (pe % 9),
      pegRatio: 0.8 + (pe % 5) * 0.3,
      dividendYield: pe % 4 === 0 ? 0.012 + (ups % 3) * 0.006 : 0,
      returnOnEquity: 0.08 + (ups % 6) * 0.035,
      returnOnAssets: 0.03 + (ups % 5) * 0.02,
      grossMargin: 0.25 + (ups % 7) * 0.05,
      operatingMargin: 0.08 + (ups % 5) * 0.035,
      netMargin: 0.05 + (ups % 4) * 0.03,
      freeCashFlow: marketCapB * 1e9 * (0.01 + (ups % 5) * 0.01),
      operatingCashFlow: marketCapB * 1e9 * (0.02 + (ups % 4) * 0.012),
